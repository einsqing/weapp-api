...
exports.weeklyVisitTrend = async function(beginDate, endDate) {
  const { accessToken } = await this.ensureAccessToken();
  const url = `https://api.weixin.qq.com/datacube/getweanalysisappidweeklyvisittrend?access_token=${accessToken}`;
  const params = {
    begin_date: beginDate,
    end_date: endDate
  };
...
exports.visitDistribution = async function(beginDate, endDate) {
  const { accessToken } = await this.ensureAccessToken();
  const url = `https://api.weixin.qq.com/datacube/getweanalysisappidvisitdistribution?access_token=${accessToken}`;
  const params = {
    begin_date: beginDate,
    end_date: endDate
  };
